...
def submit_feedback():
    ...
    return jsonify({"status": "success", "id": feedback_entry["id"]})
...
class GraphState(TypedDict):
    ...
    formatted_context_for_generation: Optional[str]

    generated_response_text: Optional[str]
    token_info: Optional[Dict[str, int]]

    critique_json: Optional[Dict[str, str]]
...
def prepare_final_output_node(state: Optional[GraphState]) -> Dict[str, Any]:
...
def should_retry_search_edge(state: Optional[GraphState]) -> str:
